...
## Two Ways to Extract Positions

### Option 1: Using /extract-positions Slash Command (Recommended)

The easiest and best way to extract positions with high-quality LLM-generated titles:
...
1. Ask for (or detect) the source file path
2. Read and extract all bullet-pointed positions
3. Generate maximally descriptive titles using LLM (max 7 words, following best practices)
4. Automatically run the extraction script with the generated titles

**Example titles generated by LLM:**
...
### Option 2: Manual Extraction with Custom Titles

You can manually call the script with pre-generated titles:

```bash
python3 .claude/skills/position-extractor/scripts/extract_positions.py "<source-file>" --titles="title1|title2|title3..."
```

**Title requirements:**
- Maximum 7 words per title
- No special characters (only Finnish letters äöåÄÖÅ, numbers, spaces, hyphens)
- Titles separated by pipe character "|"
- Number of titles must match number of positions
...
python3 .claude/skills/position-extractor/scripts/extract_positions.py \
  ...
  --titles="Datakeskukset tukemaan sähköjärjestelmän tasapainoa|Sähköverkkosuunnittelu kytketään datakeskusklustereiden kehitykseen"
```

### Option 3: Automatic Title Generation (Fallback)

Run the script without custom titles to use automatic algorithmic title generation:

```bash
python3 .claude/skills/position-extractor/scripts/extract_positions.py "<source-file>"
```

This will generate titles automatically from the first sentence of each position, but quality may not be as good as LLM-generated titles.

## Workflow

### Step 1: Choose Extraction Method

Recommend using `/extract-positions` slash command for best results.

### Step 2: Review the Output

Show the user:
- Number of positions extracted
- List of created position files with titles
- Location of the processed file

### Step 3: Optional Adjustments

If the user wants to modify:
- Position titles - Edit the individual position files in `positions-drafts/`
- Processed file structure - Edit the processed file in `processed/`
